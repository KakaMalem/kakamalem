...
/**
 * Helper function to upload a file to local storage and create media entry
 */
...
async function cleanupOrphanedMedia(mediaIds: string[]): Promise<void> {
  try {
    ...
    for (const record of mediaRecords) {
      // Extract relative path from URL (e.g., /uploads/tenants/xxx/products/file.webp)
      const relativePath = record.url.replace(/^\/uploads\//, "");
      if (relativePath) {
